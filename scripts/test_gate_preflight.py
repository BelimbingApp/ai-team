...
class GateMechanismTest(unittest.TestCase):
    ...
    def test_missing_native_approval_warns_without_reclassifying_ai_team_gate(self):
        ...
        self.assertIn("only 0 APPROVED review(s) are visible", result.stdout)
    ...
    def test_present_native_approval_is_reported_without_a_warning(self):
        ...
        self.assertEqual(result.returncode, 0, result.stdout + result.stderr)
        self.assertIn(
            "GitHub native approval preflight: requires 1, 1 APPROVED review(s) visible",
            result.stdout,
        )
        self.assertNotIn("only 0 APPROVED review(s) are visible", result.stdout)
